...
package nl.caesar.pdsm.components.resources.skin
{
 ...
 */
public class CanvasBottomEdgeSkin extends Border
{
	 ...
	 *  Constructor.
	 */
	public function CanvasBottomEdgeSkin()
	{
		super();
	}
	...
	override protected function updateDisplayList(w:Number, h:Number):void
	{
		...
		{		
			...
			case "selectedOverSkin":
			{
				...
				drawRoundRect(
					0, 0, w, h, cr,
					[ themeColor, themeColorDrk1 ], [0,0],
					verticalGradientMatrix(0, 0, w , h )); 
												
				// button fill
				drawRoundRect(
					1, 1, w - 2, h - 2, cr1,
					[ fillColors[1], fillColors[1] ], [0,0],
			...
			case "upSkin":
			{
				...
				else
				{
					...
					drawRoundRect(
						0, 0, w, h, cr,
						[ borderColor, borderColorDrk1 ], [0,0],
						verticalGradientMatrix(0, 0, w, h ),
						GradientType.LINEAR, null, 
			...
			case "selectedDownSkin":
			{
				...
				drawRoundRect(
					1, 1, w - 2, h - 2, cr1,
					[ derStyles.fillColorPress1, derStyles.fillColorPress2], [0,0],
					verticalGradientMatrix(1, 1, w - 2, h - 2)); 
		...
		graphics.moveTo(Math.floor(w/2)-7,Math.floor(h/2)-2);
		graphics.lineTo(Math.floor(w/2)+7,Math.floor(h/2)-2);
		graphics.moveTo(Math.floor(w/2)-7,Math.floor(h/2));
		graphics.lineTo(Math.floor(w/2)+7,Math.floor(h/2));
		graphics.moveTo(Math.floor(w/2)-7,Math.floor(h/2)+2);
		graphics.lineTo(Math.floor(w/2)+7,Math.floor(h/2)+2);
